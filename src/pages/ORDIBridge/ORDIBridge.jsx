...
export default function ORDIridge() {
  ...
  const { ordinalsAddress, paymentAddress,BISON_SEQUENCER_ENDPOINT,claim_endpoint,ORDI_endpoint,setORDI_endpoint,NETWORK} = useWallet();
  ...
  const [ordiResponse, setLabbResponse] = useState(null);
  const [ordiBalanceAmount, setLabbBalanceAmount] = useState(0);
  ...
  const fetchLabbBalance = async () => {
    const url = `${ORDI_endpoint}/ordi_balance`;
    try {
      if( ordinalsAddress==''){
        return;
      }
  ...
  const handlePegIn = async () => {
    ...
    const response = await fetch(`${ORDI_endpoint}/bridge_peg_in`, requestOptions);
  ...
  const checkBalance = async () => {
    ...
    if (!ordiResponse || !ordiResponse.address) return; // 如果没有  或 hash，则返回
    ...
    try {
      const response = await fetch(`${ORDI_endpoint}/peg_in_status`, requestOptions);
  ...
  const fetchContracts = async () => {
    try {
      ...
      const ordiContract = data.contracts.find(contract => contract.tick === 'ordi'||contract.tick === 'ORDI');
      if (ordiContract) {
        setORDI_endpoint(ordiContract.contractEndpoint);
        fetchBalanceForContract(ordiContract);
      }
            ...
            <XBox isBackground={true} height={isClicked ? 500 : 400}>
              ...
              >
                ...
                <select
                  ...
                  name="asset"
                >
                  <option value="">ORDI  balance: {ordiBalanceAmount} (Layer One)</option>
                </select>
              </div>
              ...
              <>
                <div style={{
                  ...
                  <p style={{
                    ...
                    <br />
                    <strong>
                      Send { data.amount} ORDI tokens <br/> ONLY from {ordinalsAddress} <br/>to {ordiResponse ? ordiResponse.address : "error"}
                      <br /> 
                      Last Deposit Status: {ordiResponse ? ordiResponse.status : ""} 
                      {ordiResponse && ordiResponse.status === "successful" && 
                          <span>
                            &nbsp;Amount: {amt/100000000} ORDI
                          </span>
                        }
                    </strong>
                  </p>
          ...
          <XBox isBackground={true} height={isClicked ? 500 : 400}>
            ...
            <label
              ...
              <p style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', }}>
                {tokenBalances['ordi'] ? (tokenBalances['ordi'] / 100000000).toFixed(8) : tokenBalances['ORDI'] ? (tokenBalances['ORDI'] / 100000000).toFixed(8) : '0.00000000'} ORDI
              </p>
